...
class SummaryHandler(cyclone.web.RequestHandler):
    def get(self, foo):
        ...
        for (router_type, nickname, fingerprint, running, country_code, time_published) in filtered_relays:
            if running == 0:
                running_str = 'false'
            if running == 1:
                running_str = 'true' 
        ...
        for (router_type, nickname, fingerprint, running, country_code, time_published) in filtered_bridges:   
            if running == 0:
                running_str = 'false'
            if running == 1:
                running_str = 'true' 
